use tcod::colors::{Color, WHITE, DESATURATED_GREEN, DARKER_GREEN, DARK_RED, DARK_GREEN};

pub const COLOR_DARK_WALL: Color = Color { r: 0, g: 0, b: 0};
pub const COLOR_DARK_BREAKABLE_WALL: Color = Color {r:25, g:25, b: 25};
pub const COLOR_LIGHT_WALL: Color = Color { r: 130, g: 110, b: 50};
pub const COLOR_LIGHT_BREAKABLE_WALL: Color = Color { r: 130, g: 130, b: 100};

pub const COLOR_DARK_GROUND: Color = Color {r: 50, g: 50, b: 150};
pub const COLOR_LIGHT_GROUND: Color = Color {r:200, g:180, b:50};

pub const COLOR_RED_BLOOD: Color = DARK_RED;
pub const COLOR_GREEN_BLOOD: Color = DARK_GREEN;
pub const COLOR_ORC: Color = DESATURATED_GREEN;
pub const COLOR_TROLL: Color = DARKER_GREEN;

pub const COLOR_WHITE: Color = WHITE;